...
export default function GlyphComponent() {
  ...
  useEffect(() => {
    const fetchConfigs = async () => {
      try {
        const res = await fetch(`${MONOGRAM_API_BASE}/glyphs`);
        if (res.ok) {
  ...
  const fetchPreview = async (configToUse?: GlyphConfig) => {
    ...
    try {
      ...
      const res = await fetch(`${MONOGRAM_API_BASE}/glyphs/preview`, {
        method: "POST",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`${MONOGRAM_API_BASE}/glyphs/save`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      ...
      if (res.ok) {
        ...
        const configRes = await fetch(`${MONOGRAM_API_BASE}/glyphs`);
        if (configRes.ok) {
