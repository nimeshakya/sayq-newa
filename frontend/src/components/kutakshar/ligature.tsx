...
export default function LigatureComponent() {
  const [slots, setSlots] = useState<LigatureSlot[]>([
    { id: 1, char: 'क', scale: 1.0, x_offset: 0, y_offset: 0, rotation: 0, skew_x: 0, skew_y: 0, crop_top: 0, crop_bottom: 0, crop_left: 0, crop_right: 0 },
    { id: 2, char: 'र', scale: 1.0, x_offset: 0, y_offset: 0, rotation: 0, skew_x: 0, skew_y: 0, crop_top: 0, crop_bottom: 0, crop_left: 0, crop_right: 0 },
  ]);
  ...
  const selectOptions = [
    ...charOptions.map(([key, char]) => ({ value: char, label: `${key} (${char})` })),
  ...
  const customStyles = {
    ...
    option: (base: any, state: any) => ({
      ...base,
      fontSize: '1.25rem',
      backgroundColor: state.isSelected ? '#8b1e3f' : state.isFocused ? 'rgba(139,30,63,0.08)' : 'white',
  ...
  const addSlot = () => {
    const newSlot: LigatureSlot = {
      id: Date.now(), char: '्',
      scale: 1.0, x_offset: 0, y_offset: 0, rotation: 0,
      skew_x: 0, skew_y: 0, crop_top: 0, crop_bottom: 0,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatus(res.ok ? '✓ Ligature rule saved successfully!' : '✗ Failed to save');
    } catch (e) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto p-4 sm:p-6">
      ...
      <div className="bg-white border border-gray-100 rounded-[32px] p-8 shadow-[0_20px_50px_rgba(0,0,0,0.05)] transition-all">
        ...
        <p className="text-gray-500 text-sm mb-10">Define custom joining rules with fine-tuned positioning.</p>

        <div className="space-y-8">
          {slots.map((slot, index) => (
            <div key={slot.id} className="border border-gray-100 rounded-3xl p-6 bg-[#f8fbff]">
              ...
              <div className="mb-6">
                <Select
                  ...
                  onChange={(opt: any) => updateSlot(slot.id, { char: opt.value })}
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-5 text-sm">
                {[
                  { label: 'Scale', key: 'scale' as const, min: 0.5, max: 2.0, step: 0.05, unit: 'x' },
                  { label: 'X-Offset', key: 'x_offset' as const, min: -80, max: 80, step: 1, unit: 'px' },
                  { label: 'Y-Offset', key: 'y_offset' as const, min: -80, max: 80, step: 1, unit: 'px' },
                  { label: 'Rotation', key: 'rotation' as const, min: -180, max: 180, step: 1, unit: '°' },
                  { label: 'Skew X', key: 'skew_x' as const, min: -1, max: 1, step: 0.05 },
                  { label: 'Skew Y', key: 'skew_y' as const, min: -1, max: 1, step: 0.05 },
                  { label: 'Crop Top', key: 'crop_top' as const, min: 0, max: 150, step: 1, unit: 'px' },
                  { label: 'Crop Bottom', key: 'crop_bottom' as const, min: 0, max: 150, step: 1, unit: 'px' },
                  { label: 'Crop Left', key: 'crop_left' as const, min: 0, max: 150, step: 1, unit: 'px' },
                  { label: 'Crop Right', key: 'crop_right' as const, min: 0, max: 150, step: 1, unit: 'px' },
                ].map(({ label, key, min, max, step, unit }) => (
                  <div key={key}>
                    <div className="flex justify-between text-xs mb-1.5 font-medium text-gray-600">
                      <span>{label}</span>
                      <span className="font-mono text-[#8b1e3f]">{(slot as any)[key]}{unit}</span>
                    </div>
                    <input
                      type="range"
                      min={min}
                      ...
                      onChange={(e) => updateSlot(slot.id, { [key]: parseFloat(e.target.value) })}
        ...
        {status && (
          <p className={`text-center mt-6 font-medium ${status.includes('✓') ? 'text-green-600' : 'text-red-600'}`}>
            {status}
          </p>
        )}
      </div>
        ...
        <div className="flex-1 bg-white border-2 border-dashed border-gray-100 rounded-[32px] flex items-center justify-center relative overflow-hidden min-h-[420px]">
          ...
          {previewUrl ? (
            ...
            <div className="text-gray-300 text-center space-y-4">
              ...
              <p className="font-bold text-lg text-gray-400">Add characters to see preview</p>
            </div>
          )}
        </div>
